...
<svelte:head><title>Benutzer – LebensmittelKumpel</title></svelte:head>

<div class="flex items-center justify-between gap-3">
	<div>
		<h1 class="text-2xl font-bold">Benutzer</h1>
		<p class="mt-1 text-sm text-gray-500">{data.users.length} Benutzer</p>
	</div>
	<button
		type="button"
...
{#if form && 'created' in form && form.created}
	<div class="mt-4 max-w-xl rounded-lg bg-green-100 px-4 py-3 text-sm text-green-800">Benutzer „{form.created}" angelegt.</div>
{/if}
...
{#if showCreate}
	...
	>
		<h2 class="text-sm font-semibold text-gray-700">Neuer Benutzer</h2>
		<div class="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
			<div>
				<label for="c-username" class="block text-xs font-medium text-gray-500">Benutzername</label>
			...
			<div>
				<label for="c-email" class="block text-xs font-medium text-gray-500">E-Mail</label>
			...
			<div>
				<label for="c-password" class="block text-xs font-medium text-gray-500">Passwort</label>
			...
			<div>
				<label for="c-role" class="block text-xs font-medium text-gray-500">Rolle</label>
		...
		<button type="submit" class="mt-3 rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white hover:bg-green-700">Anlegen</button>
	</form>
{/if}
			...
			{#if editing === user.username}
				...
				>
					...
					<div class="flex items-center justify-between">
						...
						<span class="text-xs text-gray-400">bearbeiten</span>
					</div>
					<div class="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
						<div>
							<label for={`e-email-${user.username}`} class="block text-xs text-gray-500">E-Mail</label>
						...
						<div>
							<label for={`e-role-${user.username}`} class="block text-xs text-gray-500">Rolle</label>
						...
						<div class="sm:col-span-2">
							<label for={`e-pw-${user.username}`} class="block text-xs text-gray-500">Neues Passwort (leer = unverändert)</label>
				...
				<div class="flex items-center gap-3">
					<div class="min-w-0 flex-1">
						<div class="flex items-center gap-2">
							<span class="font-medium">{user.username}</span>
							{#if user.role === 'admin'}
								<span class="rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700">Admin</span>
							{/if}
					...
					<button type="button" onclick={() => { editing = user.username; showCreate = false; }} class="shrink-0 text-sm text-gray-500 underline hover:text-gray-700">Bearbeiten</button>
					{#if user.username !== data.user?.username}
						<form method="POST" action="?/delete" use:enhance>
							<input type="hidden" name="username" value={user.username} />
							<button type="submit" class="shrink-0 text-sm text-red-600 underline hover:text-red-700">Löschen</button>
						</form>
					{/if}
				</div>
			{/if}
